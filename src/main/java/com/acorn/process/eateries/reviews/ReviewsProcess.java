...
public class ReviewsProcess {
	...
	private final MembersRepository membersMainRepository;
	...
	public Page<ReviewResponseDto> getReviewsByMemberNo(String memberNo, int page){
		...
		return reviewsRepository.getReviewsByMemberNo(pageable,memberNo)
	...
	public Page<ReviewResponseDto> getReviewsByEateryNo(String eateryNo, int page){
		Pageable pageable = PageRequest.of(page, 5);
		return reviewsRepository.getReviewsByEateryNo(pageable,eateryNo)
			.map(review -> getReviewResponseDto(review));
	}
	...
	public ReviewResponseDto getReviewResponseDto(Reviews reviews) {
		return ReviewResponseDto.builder()
			...
			.reviewImagesResponseDto(
				reviewImagesRepository.getReviewImagesByReviewNo(reviews.getNo().toString())
					.stream()
					.map(ReviewImagesResponseDto::toDto).toList()
			)
			.build();
	 ...
	 * @return
	 */
	public ReviewImages setReviewImagesRequestDtoToEntity(
		ReviewImagesRequestDto dto,
		Reviews reviews
	) {
	...
	public void registReviewImages(Reviews review,List<MultipartFile> files) throws Exception {
		for (MultipartFile file : files) {
			if (!file.isEmpty()) {
				...
				reviewImagesRepository.save(
					setReviewImagesRequestDtoToEntity(reviewImagesRequestDto, review)
				);
			}
		}
	}
	...
	public void updateReviewImage(String reviewNo, List<MultipartFile> files) throws Exception {
		...
		//저장된 경로 리스트
		List<String> existingPhotoPaths = getReviewImagesByReviewNo(reviewNo)
			.stream()
			.map(ReviewImagesResponseDto::getImageUrl)
			.toList();
	    ...
	    List<MultipartFile> filesToAdd=files.stream()
			.filter(file -> {
				String fullPath = Paths.get(
					uploadDir,
					reviewNo + "_" + file.getOriginalFilename()
				).toString();
	...
	public void updateReview(
		String reviewNo,ReviewRequestDto inputDto,
		...
		if (!files.isEmpty()) {
			...
			reviewImagesRepository.deleteAllById(
				reviewImagesRepository.findAllNoByReviewNo(reviewNo)
			);
		}
	}
